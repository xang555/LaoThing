...
const FirebaseError FirebaseCloudMessaging::SendPayload(
    const char* payload) {
    String httpsFingerprint = "98 32 A0 39 5B 48 30 12 E1 A9 4A 51 51 4B 8C B8 83 9F 86 32";
    HTTPClient http;
    http.begin("https://fcm.googleapis.com/fcm/send", httpsFingerprint);
  ...
  if (status != 200) {
    return FirebaseError(status,  HTTPClient::errorToString(status).c_str());
  } else {
